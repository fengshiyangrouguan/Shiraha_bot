...
from src.common.utils.image_interpret import interpret_image, interpret_sticker
    ...
    async def process_to_context(self):
        ...
        for seg in self.segments:
            if seg.type == "text":
                ...
                try:
                    description = await interpret_image(base64_image_data=seg.data,)
                    texts.append(description)
                except Exception as e:
                        texts.append(f"[{seg.type}]")
                        logger.error(f"调用 interpret_image 失败: {e}", exc_info=True)
            elif seg.type == "sticker":
                try:
                    description = await interpret_sticker(base64_image_data=seg.data,)
                    texts.append(description)
                except Exception as e:
                    texts.append(f"[{seg.type}]")
                    logger.error(f"调用 interpret_sticker 失败: {e}", exc_info=True)
            else:
                # 其它类型，用占位表示
                texts.append(f"[{seg.type}]")
