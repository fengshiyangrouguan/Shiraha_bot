...
class TaskManager:
    ...
    async def create_task(self, cortex: str, target_id: str, priority: int, motive: str) -> TaskInstance:
        """创建一个新进程"""
        ...
        new_task = TaskInstance(
            ...
            priority=priority,
            motive=motive,
            context_ref=f"ctx_{target_id}_{int(new_task.created_at)}" # 自动生成上下文引用
        )
