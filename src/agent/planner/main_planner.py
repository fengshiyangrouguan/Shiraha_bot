...
class MainPlanner(BasePlanner):
    ...
    主规划器。
    负责将高阶意图分解为具体的行动步骤。
    它声明自己需要一个 `intent` 字符串作为输入。
    """
    def __init__(self):
        super().__init__(task_name="main_planner")
    ...
    def _build_prompt(self) -> List[Dict[str, Any]]:
        ...
        """
        motive = self.world_model.motive
        if not motive:
            raise ValueError("在 _build_prompt 中未提供 'intent' 参数。")
        ...
        system_prompt = (
            ...
            f"```\n"
            "如果没有合适的工具，或者你认为当前意图不需要任何操作，请在action中返回 `{\"tool_name\": \"no_op\", \"parameters\": {}}`。"
        )
        ...
        return builder.get_message_dict()
